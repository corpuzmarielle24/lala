...
<nav class="main-header  navbar-expand navbar-white navbar-light " style="padding: 5px !important;z-index: 1">
  ...
  <div class="row mx-0">
    ...
    <div class="col-sm-8">
      <h4 style="font-weight: bold" class="mt-1">Found Pets
      </h4>
    </div>
  </div>
                  ...
                  @endphp
                    ...
                    <tr>
                      ...
                      <td class="align-middle text-wrap">
                        @if($user->image1)
                        <a href="{{ asset('uploads/pet/'.$user->image1.'') }}" target="_blank">
                            <img src="{{ asset('uploads/pet/'.$user->image1.'') }}" style="height: 80px; width: 120px;" class="card-img-top" alt="...">
                        </a>
                        @endif
                    </td>

                      <td class="align-middle text-wrap" >{{$user ->pet_name}}</td>


                    <td class="align-middle text-wrap" >{{$user ->breed}}</td>
                      <td class="align-middle text-wrap" >{{$user ->gender}}</td>
                      <td class="align-middle text-wrap" >{{$user ->age}}</td>

                      <td class="align-middle text-wrap" >{{$user ->description}}</td>
                      <td class="align-middle text-wrap" >{{$user ->last_address}}</td>
                      <td class="align-middle text-wrap" >{{$user ->owner_name}}</td>
                      <td class="align-middle text-wrap" >{{$user ->email}}</td>
                      <td class="align-middle text-wrap" >{{$user ->phone}}</td>
                      <td class="align-middle text-wrap" >
               <form action="/admin/approved/{{$user ->id }}" method="POST" enctype="multipart/form-data" style="display: inline;" >
                            @csrf 
                            @method('PUT')
                            <input type="text" style="display: none;" value="1" name="is_deleted" id="is_deleted">
                            <button onclick="return confirm('Are you sure you want to delete this report?')" type="submit"  class="btn  btn-danger" style="display:inline;"><i class="fa fa-trash"> </i>Delete</button>
                        </form>
               </td>
